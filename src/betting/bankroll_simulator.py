...
class BankrollSimulator:
    ...
    def _mostrar_resultados(self, resultados, df_flat, df_kelly):
        ...
        if diff_ganancia > 0:
            ...
            mejora_pct = (
                (diff_ganancia / abs(resultados["flat"]["ganancia"])) * 100
                if resultados["flat"]["ganancia"] != 0
                else float("inf")
            )
    ...
    def _visualizar_comparacion(self, hist_flat, hist_kelly, resultados):
        ...
        ax1.plot(
            hist_kelly, label="Kelly Criterion (25%)", linewidth=2.5, color="#2ecc71", alpha=0.8
        )
        ax1.axhline(
            y=self.bankroll_inicial,
            linestyle="--",
            color="red",
            alpha=0.5,
            linewidth=1.5,
