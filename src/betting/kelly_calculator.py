...
class KellyCalculator:
    ...
    def calcular_con_limites(self, prob, cuota, bankroll, 
                             min_apuesta=5, max_apuesta_pct=0.05):
        """
    ...
    def comparar_estrategias(self, prob, cuota, bankroll=1000, apuesta_flat=10):
        ...
        print(f"\n💰 Tamaño de Apuesta:")
        print(f"   Flat Betting:  {apuesta_flat:.2f}€ ({apuesta_flat/bankroll*100:.1f}% del bankroll)")
        print(f"   Kelly (25%):   {apuesta_kelly:.2f}€ ({apuesta_kelly/bankroll*100:.1f}% del bankroll)")
    ...
    def analizar_sensibilidad(self, cuota, bankroll=1000):
        ...
        for prob in np.arange(0.45, 0.75, 0.05):
            ...
            print(f"{prob*100:5.1f}%   {ev*100:+6.2f}%   {kelly_full*100:8.2f}%   "
