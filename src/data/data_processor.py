...
def explorar_datos(df):
    ...
    logger.info("\n🏆 Estadísticas de rankings:")
    logger.info(
        f"   Ranking ganador - Media: {df['winner_rank'].mean():.1f}, Mediana: {df['winner_rank'].median():.1f}"
    )
    logger.info(
        f"   Ranking perdedor - Media: {df['loser_rank'].mean():.1f}, Mediana: {df['loser_rank'].median():.1f}"
    )
