...
def actualizar_datos_completos():
    ...
    if datos:
        ...
        df_completo["tourney_date"] = pd.to_datetime(
            df_completo["tourney_date"], format="%Y%m%d", errors="coerce"
        )
        logger.info("\n📊 Partidos por año:")
        for year in years:
...
def verificar_compatibilidad():
    ...
    logger.info(
        f"   Rankings nulos: winner={df_tml['winner_rank'].isna().sum()}, loser={df_tml['loser_rank'].isna().sum()}"
    )
