...
async def create_match_and_predict(request: MatchCreateRequest):
    ...
    try:
        ...
        logger.info(f"✅ Partido creado y predicción generada: {request.jugador1_nombre} vs {request.jugador2_nombre}")
...
async def update_match_result(match_id: int, request: MatchResultRequest):
    ...
    try:
        ...
        if bet_updated:
            ...
            cursor.execute("SELECT * FROM bets WHERE match_id = ? AND estado = 'completada'", (match_id,))
            bet = cursor.fetchone()
...
async def refresh_match_odds(match_id: int, jugador1_cuota: float, jugador2_cuota: float):
    ...
    try:
        ...
        if prediccion_anterior:
            ...
            mejor_opcion_anterior = prediccion_anterior.get('mejor_opcion')
            ...
            if recomendacion_anterior != recomendacion_nueva:
                ...
                nueva_recomendacion_info = {
                    ...
                    "nota": "Cambio detectado - apuesta original se mantiene" if bet_existente else "Nueva oportunidad de apuesta"
                }
                ...
                if not bet_existente and mejor_opcion_nueva is not None:
                    ...
                    apuesta_actual = {
                        "jugador_apostado": mejor_opcion_nueva,
                        "cuota_apostada": jugador1_cuota if mejor_opcion_nueva == partido['jugador1_nombre'] else jugador2_cuota,
                        "stake": kelly_j1 or kelly_j2 or 10.0,
                        "prediction_version": prediccion_anterior['version'] + 1 if prediccion_anterior else 2,
    ...
    try:
        ...
        for pred in predicciones:
            version_data = {
                ...
                "cuotas": {
                    "jugador1": pred['jugador1_cuota'],
                    "jugador2": pred['jugador2_cuota']
                },
                "probabilidades": {
        ...
        for i in range(1, len(versiones)):
            ...
            cambio_cuota_j1 = abs(curr['cuotas']['jugador1'] - prev['cuotas']['jugador1']) / prev['cuotas']['jugador1']
            cambio_cuota_j2 = abs(curr['cuotas']['jugador2'] - prev['cuotas']['jugador2']) / prev['cuotas']['jugador2']
...
async def github_webhook(request: Request):
    ...
    try:
        ...
        logger.info(f"🔔 Webhook recibido: Commit {commit_info.get('sha')} - {commit_info.get('message')}")
    ...
    try:
        ...
        def check_github_commits():
            ...
            try:
                ...
                if new_commit:
                    logger.info(f"🆕 Commit nuevo detectado en TML-Database: {new_commit['short_sha']}")
