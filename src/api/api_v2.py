                ...
                for p in partidos_raw:
                    ...
                    if live_api and live_api.get("event_live") == "1":
                        if _is_match_still_live(live_api):
                            ...
                            eg = live_api.get("event_game_result")
                            es = live_api.get("event_serve")
                            if swapped_order and (eg or es):
                                # API first/second = nuestro jugador2/jugador1: intercambiar
                                if eg and "-" in str(eg):
...
async def get_match_details(
    ...
    devuelve exactamente la misma información que `GET /matches/{id}/full`.
    """
...
# ============================================================
# ADMIN / CRON (solo endpoints usados por cron-job.org)
# ============================================================
...
async def trigger_retraining():
    """
    GET para crons externos: dispara sync de cuotas y predicciones para partidos pendientes.
...
async def startup_event():
    ...
    try:
        ...
        logger.info("   - ELO: desde CSV en datos/raw/ (actualizar vía GET /cron/refresh-elo o redeploy)")
