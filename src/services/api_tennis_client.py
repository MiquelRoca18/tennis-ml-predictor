        ...
        try:
            ...
            if "X-RateLimit-Remaining" in response.headers:
                ...
                if self.requests_remaining < 10:
                    logger.warning(
                        f"⚠️  ADVERTENCIA: Solo quedan {self.requests_remaining} requests"
                    )
...
if __name__ == "__main__":
    ...
    for match in matches[:5]:  # Mostrar primeros 5
        ...
        print(
            f"👤 {match['player1_name']} (#{match.get('player1_ranking', 'N/A')}) - {match.get('player1_odds', 'N/A')}"
        )
        print(f"   vs")
        print(
            f"👤 {match['player2_name']} (#{match.get('player2_ranking', 'N/A')}) - {match.get('player2_odds', 'N/A')}"
        )
