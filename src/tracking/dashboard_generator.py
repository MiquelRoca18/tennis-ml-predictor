...
class DashboardGenerator:
    ...
    def _crear_distribucion_win_rate(self, df):
        ...
        fig.update_layout(title=f"Win Rate: {ganadas/(ganadas+perdidas)*100:.1f}%", height=400)
    ...
    def _crear_distribucion_ev(self, df):
        ...
        fig.add_trace(
            go.Histogram(
                x=df["ev"] * 100, nbinsx=20, marker_color="#3498db", name="EV Distribution"
            )
        )
    ...
    def _crear_ev_vs_resultado(self, df):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=df["ganancia"],
                mode="markers",
                marker=dict(size=10, color=colors, opacity=0.6, line=dict(width=1, color="white")),
...
<body>
    ...
    def _crear_tabla_ultimas_apuestas(self, df):
        ...
        for _, row in df_ultimas.iterrows():
            resultado_icon = (
                "✅" if row["resultado_real"] == 1 else "❌" if row["resultado_real"] == 0 else "⏳"
            )
