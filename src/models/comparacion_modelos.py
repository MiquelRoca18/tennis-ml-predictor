...
class ModelComparator:
    ...
    def entrenar_y_evaluar(self, X_train, y_train, X_val, y_val, X_test, y_test):
        ...
        for nombre, modelo in self.modelos.items():
            ...
            try:
                ...
                logger.info(
                    f"   Accuracy: Train {resultados['accuracy_train']*100:.2f}% | "
                    f"Val {resultados['accuracy_val']*100:.2f}% | "
                    f"Test {resultados['accuracy_test']*100:.2f}%"
                )
                logger.info(
                    f"   Brier:    Train {resultados['brier_train']:.4f} | "
                ...
                try:
                    ...
                    modelo_calibrado = CalibratedClassifierCV(
                        modelo_base_calibrado, method="isotonic", cv=5
                    )
    ...
    def comparar_resultados(self):
        ...
        logger.info(
            f"   Accuracy Test: {df_resultados.loc[mejor_modelo, 'accuracy_test']*100:.2f}%"
        )
        logger.info(f"   Brier Score Test: {df_resultados.loc[mejor_modelo, 'brier_test']:.4f}")
        logger.info(
            f"   Brier Calibrado: {df_resultados.loc[mejor_modelo, 'brier_test_calibrado']:.4f}"
        )
...
if __name__ == "__main__":
    ...
    resultados = comparador.entrenar_y_evaluar(X_train, y_train, X_val, y_val, X_test, y_test)
