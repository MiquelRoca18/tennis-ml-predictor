...
def weighted_ensemble_prediction(X_test, y_test):
    ...
    logger.info(f"   Accuracy RF:  {acc_rf*100:.2f}% → Ensemble: {accuracy*100:.2f}% ({(accuracy-acc_rf)*100:+.2f}%)")
    logger.info(f"   Accuracy XGB: {acc_xgb*100:.2f}% → Ensemble: {accuracy*100:.2f}% ({(accuracy-acc_xgb)*100:+.2f}%)")
    logger.info(f"   Accuracy GB:  {acc_gb*100:.2f}% → Ensemble: {accuracy*100:.2f}% ({(accuracy-acc_gb)*100:+.2f}%)")
