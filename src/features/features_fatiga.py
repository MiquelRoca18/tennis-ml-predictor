...
class FatigaCalculator:
    ...
    def calcular_fatiga(self, jugador_nombre, fecha_partido):
        ...
        fatiga = {
            ...
            'minutos_por_partido_7d': float(minutos_7d / len(partidos_7d)) if len(partidos_7d) > 0 else 0.0,
    ...
    def calcular_ventaja_fatiga(self, jugador1_nombre, jugador2_nombre, fecha_partido):
        ...
        ventaja = {
            # Diferencias
            'diff_dias_descanso': fatiga_j1['dias_desde_ultimo_partido'] - fatiga_j2['dias_desde_ultimo_partido'],
            'diff_partidos_7d': fatiga_j1['partidos_ultimos_7d'] - fatiga_j2['partidos_ultimos_7d'],
            'diff_partidos_14d': fatiga_j1['partidos_ultimos_14d'] - fatiga_j2['partidos_ultimos_14d'],
            'diff_minutos_7d': fatiga_j1['minutos_ultimos_7d'] - fatiga_j2['minutos_ultimos_7d'],
            'diff_carga_score': fatiga_j1['carga_reciente_score'] - fatiga_j2['carga_reciente_score'],
            
            # Indicadores
            'j1_mas_fresco': 1 if fatiga_j1['dias_desde_ultimo_partido'] > fatiga_j2['dias_desde_ultimo_partido'] else 0,
            'j1_mas_activo': 1 if fatiga_j1['partidos_ultimos_14d'] > fatiga_j2['partidos_ultimos_14d'] else 0,
            
            # Situaciones especiales
            'ambos_cansados': 1 if (fatiga_j1['torneo_largo'] == 1 and fatiga_j2['torneo_largo'] == 1) else 0,
            'j1_torneo_largo_j2_no': 1 if (fatiga_j1['torneo_largo'] == 1 and fatiga_j2['torneo_largo'] == 0) else 0
        }
