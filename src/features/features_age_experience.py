...
class AgeExperienceCalculator:
    ...
    def _calcular_estadisticas_carrera(self):
        ...
        for _, row in self.df.iterrows():
            ...
            all_matches.append({
                'jugador': row[winner_col],
                'fecha': row['fecha'],
                'resultado': 1
            })
            all_matches.append({
                'jugador': row[loser_col],
                'fecha': row['fecha'],
                'resultado': 0
            })
        ...
        for jugador in matches_df['jugador'].unique():
            ...
            jugador_matches['win_rate_career'] = jugador_matches['victorias_career'] / jugador_matches['partidos_career']
            
            # Primera aparición (debut)
            primera_fecha = jugador_matches['fecha'].min()
            jugador_matches['anos_tour'] = (jugador_matches['fecha'] - primera_fecha).dt.days / 365.25
    ...
    def calcular_features_edad(self, partido_row):
        ...
        if pd.notna(j1_edad) and pd.notna(j2_edad):
            ...
            # Categorías de edad
            features['j1_edad_prime'] = 1 if self.PRIME_AGE_MIN <= j1_edad <= self.PRIME_AGE_MAX else 0
            features['j2_edad_prime'] = 1 if self.PRIME_AGE_MIN <= j2_edad <= self.PRIME_AGE_MAX else 0
            ...
            features['ambos_prime'] = 1 if (features['j1_edad_prime'] and features['j2_edad_prime']) else 0
            features['joven_vs_veterano'] = 1 if (features['j1_joven'] and features['j2_veterano']) else 0
            features['veterano_vs_joven'] = 1 if (features['j1_veterano'] and features['j2_joven']) else 0
            
        else:
            # Si no hay datos de edad, usar valores por defecto
            for key in ['j1_edad', 'j2_edad', 'diff_edad', 'edad_promedio',
                       'j1_edad_prime', 'j2_edad_prime', 'j1_joven', 'j2_joven',
                       'j1_veterano', 'j2_veterano', 'j1_mas_joven', 'ambos_prime',
                       'joven_vs_veterano', 'veterano_vs_joven']:
    ...
    def calcular_features_experiencia(self, jugador_nombre, oponente_nombre, fecha_partido):
        ...
        features['diff_partidos_career'] = features['j1_partidos_career'] - features['j2_partidos_career']
        features['diff_anos_tour'] = features['j1_anos_tour'] - features['j2_anos_tour']
        features['diff_win_rate_career'] = features['j1_win_rate_career'] - features['j2_win_rate_career']
        
        # Features categóricas
        features['j1_mas_experimentado'] = 1 if features['j1_partidos_career'] > features['j2_partidos_career'] else 0
        ...
        features['novato_vs_veterano'] = 1 if (features['j1_novato'] and features['j2_partidos_career'] > 500) else 0
    ...
    def calcular_features_interaccion(self, features_edad, features_exp, partido_row):
        ...
        if winner_rank_col in partido_row and pd.notna(partido_row[winner_rank_col]):
            features['j1_edad_x_rank'] = features_edad.get('j1_edad', 27) * np.log1p(partido_row[winner_rank_col])
        else:
            features['j1_edad_x_rank'] = 0
        
        if loser_rank_col in partido_row and pd.notna(partido_row[loser_rank_col]):
            features['j2_edad_x_rank'] = features_edad.get('j2_edad', 27) * np.log1p(partido_row[loser_rank_col])
        else:
            features['j2_edad_x_rank'] = 0
        
        # Experiencia × Edad (ratio)
        if features_edad.get('j1_edad', 0) > 0:
            features['j1_exp_por_edad'] = features_exp.get('j1_partidos_career', 0) / features_edad.get('j1_edad', 27)
        else:
            features['j1_exp_por_edad'] = 0
        
        if features_edad.get('j2_edad', 0) > 0:
            features['j2_exp_por_edad'] = features_exp.get('j2_partidos_career', 0) / features_edad.get('j2_edad', 27)
        else:
            features['j2_exp_por_edad'] = 0
        
        # Años en tour × Win rate career
        features['j1_anos_x_winrate'] = features_exp.get('j1_anos_tour', 0) * features_exp.get('j1_win_rate_career', 0.5)
        features['j2_anos_x_winrate'] = features_exp.get('j2_anos_tour', 0) * features_exp.get('j2_win_rate_career', 0.5)
