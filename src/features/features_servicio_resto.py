...
class ServicioRestoCalculator:
    ...
    def calcular_estadisticas_servicio(self, jugador_nombre, fecha_partido, ventana_dias=90):
        ...
        # Aces
        total_aces = (
            partidos_ganados["w_ace"].fillna(0).sum() + partidos_perdidos["l_ace"].fillna(0).sum()
        )

        # Dobles faltas
        total_df = (
            partidos_ganados["w_df"].fillna(0).sum() + partidos_perdidos["l_df"].fillna(0).sum()
        )

        # 1er servicio
        total_1stIn = (
            partidos_ganados["w_1stIn"].fillna(0).sum()
            + partidos_perdidos["l_1stIn"].fillna(0).sum()
        )
        total_1stWon = (
            partidos_ganados["w_1stWon"].fillna(0).sum()
            + partidos_perdidos["l_1stWon"].fillna(0).sum()
        )

        # 2do servicio
        total_2ndWon = (
            partidos_ganados["w_2ndWon"].fillna(0).sum()
            + partidos_perdidos["l_2ndWon"].fillna(0).sum()
        )
        puntos_2do_servicio = total_puntos_servicio - total_1stIn

        # Break points
        total_bpFaced = (
            partidos_ganados["w_bpFaced"].fillna(0).sum()
            + partidos_perdidos["l_bpFaced"].fillna(0).sum()
        )
        total_bpSaved = (
            partidos_ganados["w_bpSaved"].fillna(0).sum()
            + partidos_perdidos["l_bpSaved"].fillna(0).sum()
        )
        ...
        stats = {
            ...
            # 1er servicio
            "first_serve_in_pct": (
                total_1stIn / total_puntos_servicio if total_puntos_servicio > 0 else 0.62
            ),
            "first_serve_won_pct": total_1stWon / total_1stIn if total_1stIn > 0 else 0.68,
            # 2do servicio
            "second_serve_won_pct": (
                total_2ndWon / puntos_2do_servicio if puntos_2do_servicio > 0 else 0.48
            ),
            # Break points
    ...
    def calcular_estadisticas_resto(self, jugador_nombre, fecha_partido, ventana_dias=90):
        ...
        bp_converted = (
            partidos_ganados["l_bpFaced"].fillna(0) - partidos_ganados["l_bpSaved"].fillna(0)
        ).sum() + (
            partidos_perdidos["w_bpFaced"].fillna(0) - partidos_perdidos["w_bpSaved"].fillna(0)
        ).sum()
        ...
        stats = {
            ...
            "return_quality_score": (
                bp_converted / bp_opportunities if bp_opportunities > 0 else 0.35
            ),
        }
...
if __name__ == "__main__":
    ...
    serv = calc.calcular_estadisticas_servicio(
        jugador_nombre=jugador_ejemplo, fecha_partido=fecha_ejemplo
    )
