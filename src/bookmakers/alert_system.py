...
class AlertSystem:
    ...
    def __init__(self, email_enabled=None):
        ...
        """
        self.email_enabled = email_enabled if email_enabled is not None else BookmakerConfig.EMAIL_ENABLED
        ...
        print(f"   Email: {'✅ Habilitado' if self.email_enabled else '❌ Deshabilitado (solo consola)'}")
    ...
    def verificar_oportunidades(self, resultados_prediccion, umbral_ev=None):
        ...
        oportunidades = [
            r for r in resultados_prediccion 
            if r.get('ev', 0) > umbral
        ]
