...
class PredictorDiario:
        ...
        for idx, partido in df_partidos.iterrows():
            try:
                ...
                prediccion = {
                    ...
                    'decision': 'APOSTAR ✅' if resultado.get('expected_value', 0) > Config.EV_THRESHOLD else 'NO APOSTAR ❌',
                ...
                logger.info(f"  {jugador1} vs {jugador2}: P={resultado.get('probabilidad', 0):.2%}, EV={ev:.2%}")
    ...
    def enviar_alertas(self, predicciones: list):
        ...
        predicciones_altas = [
            ...
            if p['expected_value'] > Config.EV_THRESHOLD_ALERT
        ]
        ...
        try:
            ...
            alert_system.enviar_email(
                asunto=f"🎾 {len(predicciones_altas)} Predicciones de Alto Valor",
                mensaje=mensaje
            )
...
def main():
    ...
    parser.add_argument('--dry-run', action='store_true', 
                       help='No guardar en base de datos (solo simular)')
    parser.add_argument('--date', type=str, 
                       help='Fecha a procesar (formato: YYYY-MM-DD)')
