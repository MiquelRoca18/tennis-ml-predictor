...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/actualizacion_semanal.log"), logging.StreamHandler()],
)
...
class ActualizadorSemanal:
    ...
    def combinar_datos(self):
        ...
        logger.info(
            f"📊 Partidos después de filtrar (últimos {self.ventana_años} años): {len(df_filtered):,}"
        )
        ...
        logger.info(
            f"   Rango: {df_filtered['tourney_date'].min()} - {df_filtered['tourney_date'].max()}"
        )
    ...
    def regenerar_dataset(self):
        ...
        logger.info(
            f"✅ Dataset regenerado: {len(df_features):,} partidos, {len(df_features.columns)-2} features"
        )
    ...
    def ejecutar_actualizacion_completa(self):
        ...
        try:
            ...
            logger.info(
                f"Próxima actualización: {(datetime.now() + timedelta(days=7)).strftime('%Y-%m-%d')}"
            )
