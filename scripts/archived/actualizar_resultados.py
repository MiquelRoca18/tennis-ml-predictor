...
class ActualizadorResultados:
    ...
    def obtener_resultados(self, fecha: date) -> pd.DataFrame:
        ...
        try:
            ...
            df = self.downloader.download_matches(start_date=fecha_str, end_date=fecha_siguiente)
    ...
    def actualizar_predicciones(self, df_resultados: pd.DataFrame, fecha: date) -> dict:
        ...
        stats = {"actualizadas": 0, "no_encontradas": 0, "ganadas": 0, "perdidas": 0}
        ...
        for idx, pred in predicciones_fecha.iterrows():
            try:
                ...
                if not self.dry_run:
                    self.db.actualizar_resultado(prediccion_id=pred["id"], resultado_real=resultado)
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="No actualizar base de datos (solo simular)"
    )
    parser.add_argument(
        "--date", type=str, help="Fecha a procesar (formato: YYYY-MM-DD, default: ayer)"
    )
    ...
    # Ejecutar
    try:
        actualizador = ActualizadorResultados(db_path=args.db, dry_run=args.dry_run)
