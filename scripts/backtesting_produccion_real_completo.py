...
def descargar_datos_automaticamente(año):
    ...
    if not partidos_file.exists():
        ...
        url_partidos = f"https://raw.githubusercontent.com/Tennismylife/TML-Database/master/{año}.csv"
        try:
...
class ProductionFeatureGenerator:
    ...
    def actualizar_con_partido(self, winner_name, loser_name, surface, winner_rank, loser_rank, fecha):
        """
...
class BacktestingProduccionReal:
    ...
    def __init__(self, modelo_path, bankroll_inicial=1000.0, kelly_fraction=0.05, umbral_ev=0.10, 
                 max_cuota=2.0, min_probabilidad=0.60):
        ...
        for idx, partido_odds in df_odds.iterrows():
            ...
            try:
                ...
                feature_gen.actualizar_con_partido(
                    j1_nombre, j2_nombre, superficie,
            ...
            if (ev_j1 > self.umbral_ev and ev_j1 > ev_j2 and 
                cuota_j1 < self.max_cuota and prob_j1_gana > self.min_probabilidad):
                # Apostar a jugador 1
            ...
            elif (ev_j2 > self.umbral_ev and 
                  cuota_j2 < self.max_cuota and (1 - prob_j1_gana) > self.min_probabilidad):
                # Apostar a jugador 2
            ...
            if len(apuestas_realizadas) > 0 and len(apuestas_realizadas) % 50 == 0:
                ...
                logger.info(f"  Bankroll: {bankroll_actual:.2f}€ ({(bankroll_actual/self.bankroll_inicial-1)*100:+.1f}%)\n")
    ...
    def guardar_resultados(self, df_apuestas, bankroll_history):
        ...
        pd.DataFrame({
            'apuesta_num': range(len(bankroll_history)),
